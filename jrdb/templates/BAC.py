...
def nth_occurrence(se: pd.Series):
    ...
    return (
        se.str.extract(r"([0-9]+)", expand=False)
        .astype(float)
        .astype("Int64")
...
class BAC(Template, ProgramRaceLoadMixin):
    ...
    items = [
        ...
        ChoiceItem(
            "内外", 1, 26, "jrdb.Race.course_inout", choices.COURSE_INOUT.options()
        ),
        ...
        ChoiceItem(
            "重量", 1, 34, "jrdb.Race.impost_class", choices.IMPOST_CLASS.options()
        ),
        ...
        ChoiceItem(
            "コース", 1, 96, "jrdb.Race.course_label", choices.COURSE_LABEL.options()
        ),
        ChoiceItem(
            "開催区分", 1, 97, "jrdb.Race.host_category", choices.HOST_CATEGORY.options()
        ),
