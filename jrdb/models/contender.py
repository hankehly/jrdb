...
class Contender(BaseModel):
    ...
    weight = models.PositiveSmallIntegerField('馬体重')
    weight_diff = models.SmallIntegerField('馬体重増減', null=True)

    run_style = models.CharField('レース脚質', max_length=255, choices=choices.RUNNING_STYLE.CHOICES())
    purse = models.FloatField()
    pace_flow = models.ForeignKey('jrdb.PaceFlowCode', models.CASCADE, verbose_name='本賞金', null=True)
    c4_race_line = models.CharField('４角コース取り', max_length=255, choices=choices.RACE_LINE.CHOICES(), help_text='1:最内,2:内,3:中,4:外,5:大外')

    # KYIから取得（前日系）
    prel_jockey_idx = models.FloatField('騎手指数', help_text='基準オッズと騎手の連対率の関係を基に算出された指数値')
    prel_info_idx = models.FloatField('情報指数', help_text='基準オッズ、厩舎指数、調教指数等様々な情報を基に算出された指数値')
    prel_total_idx = models.FloatField('総合指数', help_text='ＩＤＭ、騎手指数、情報指数を合計した値')
    prel_pop_idx = models.PositiveSmallIntegerField('人気指数', null=True, help_text='その馬の人気を指数化した値')
    prel_trainer_idx = models.FloatField('調教指数')
    prel_stable_idx = models.FloatField('厩舎指数')
    flat_out_run_idx = models.SmallIntegerField('激走指数')
    ...
    dist_apt = models.PositiveSmallIntegerField('距離適性')
    prel_improvement = models.CharField('前日上昇度コード', max_length=255, choices=choices.IMPROVEMENT.CHOICES())
    odds_win_base = models.FloatField('基準オッズ')
    pop_win_base = models.PositiveSmallIntegerField('基準人気順位')
    odds_show_base = models.FloatField('基準複勝オッズ')
    pop_show_base = models.PositiveSmallIntegerField('基準複勝人気順位')
    sym_sp_c_dbl = models.PositiveSmallIntegerField('特定情報◎')
    sym_sp_c = models.PositiveSmallIntegerField('特定情報○')
    sym_sp_t_dark = models.PositiveSmallIntegerField('特定情報▲')
    sym_sp_t = models.PositiveSmallIntegerField('特定情報△')
    sym_sp_x = models.PositiveSmallIntegerField('特定情報×')
    sym_total_c_dbl = models.PositiveSmallIntegerField('総合情報◎')
    sym_total_c = models.PositiveSmallIntegerField('総合情報○')
    sym_total_t_dark = models.PositiveSmallIntegerField('総合情報▲')
    sym_total_t = models.PositiveSmallIntegerField('総合情報△')
    sym_total_x = models.PositiveSmallIntegerField('総合情報×')
    trainer_outlook = models.CharField('調教矢印コード', max_length=255, choices=choices.TRAINER_HORSE_EVALUATION.CHOICES())
    stable_outlook = models.CharField('厩舎評価コード', max_length=255, choices=choices.STABLE_HORSE_EVALUATION.CHOICES())
    jockey_exp_1o2_place_rate = models.FloatField('騎手期待連対率', help_text='騎手Ａが単勝基準オッズＢの馬に乗った場合の過去の成績を集計し、算出された連対率')
    paddock_observed_hoof = models.CharField('蹄コード', max_length=255, choices=choices.PADDOCK_OBSERVED_HOOF.CHOICES())
    yield_track_apt = models.CharField('重適正コード', max_length=255, choices=choices.YIELDING_TRACK_APTITUDE.CHOICES())
    blinker = models.CharField('ブリンカー', max_length=255, choices=choices.BLINKER.CHOICES())
    post_position = models.PositiveSmallIntegerField('枠番')
